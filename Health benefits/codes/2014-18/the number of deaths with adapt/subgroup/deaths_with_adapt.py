...
"""
1. Read the files of RR and adaptation proportion for each wealth subgroup separately
2. Concatenate two files with code
3. Calculate the number of deaths for each subgroups with adapt
"""
    ...
    for w in range(5):
        ...
        for k in range(35):
            if df.loc[k, 'RR_wealth'] < 1:
                res.loc[k, 'death' + str(w)] = ((df.loc[k, 'RR_wealth'] - 1)/(df.loc[k, 'RR_wealth'])) * 6494 * 229.0282
            else:
                res.loc[k, 'death' + str(w)] = (1 - df.loc[k, 'w' + str(w)]) * (
                    (df.loc[k, 'RR_wealth'] - 1)/(df.loc[k, 'RR_wealth'])) * 6494 * 229.0282
        res['death' + str(w)] = res['death' + str(w)].fillna(0).round().astype(int)
    print(res)
    res.to_csv('/Users/cbhbook/Desktop/death_' + flag + '.csv')
